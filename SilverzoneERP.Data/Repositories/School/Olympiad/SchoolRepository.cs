...
namespace SilverzoneERP.Data
{
    public class SchoolRepository:BaseRepository<School>,ISchoolRepository
    {
        ...
        public dynamic Search(string Anything, string SchoolCode, string SchoolName, Nullable<long> CountryId, Nullable<long> ZoneId, Nullable<long> StateId, Nullable<long> DistrictId, Nullable<long> CityId, Nullable<long> PinCode, Nullable<bool> BlackListed, Nullable<int> Event_Type, Nullable<long> EventId, int StartIndex, int Limit,ref int count)
        {
            ...
            else
            {
                if (Event_Type != null)
                {
                    if (Event_Type == (int)EventType.CurrentEvent)
                        ...
                        data = data.Where(x => x.EventManagement.Where(ec => ec.EventManagementYear == ServerKey.Event_Current_Year).Count() != 0);

                    else if (Event_Type == (int)EventType.Last_Year_Current_Event)
                        data = data.Where(x => x.EventManagement
                        .Where(ec =>
                                    ec.Event.Id ==  EventId &&
                                    ec.EventManagement_YearCode < ServerKey.Event_Current_YearCode
                              ).Count() != 0);
            ...
            count = data.Count();
            return data.OrderBy(x=>x.SchCode).Skip(StartIndex).Take(Limit).Select(x => new {
                SchId = x.Id,
                x.SchCode,
                x.SchName,
                x.SchAddress,
        ...
        public bool Exist(long SchoolCode)
        {
            return _dbset.Any(x => x.SchCode == SchoolCode);
        }
        public List<long> GetSchoolCodeList()
        {
            return _dbset.Select(x=>x.SchCode).ToList();
        }
        public dynamic Get_City_AutoFill(string CityName)
        {

            var data = _dbContext.Cities.Where(x => x.CityName.Contains(CityName) && x.Status == true).Take(10).Select(x => new
            {
                x.Id,
                Name=x.CityName,
                x.District.DistrictName,
