...
namespace SilverzoneERP.Data
{
    public class FeePaymentRepository : BaseRepository<FeePayment>, IFeePaymentRepository
    {
        ...
        public dynamic MiniStatement(long SchoolId, PaymentAgainst _paymentAgainst)
        {
            ...
            var data= _dbContext.EventManagements.Where(x => x.SchId == SchoolId && x.Status == true).ToList().Select(x => new
            {
                ...
                {
                    fp.Id,
                    fp.EventmanagementId,
                    Mode=fp.Mode.ToString(),
                    fp.DepositOnBank.AccountNo,                   
                    fp.MICRCode,
                    fp.ReceiptNo,
                    fp.ReceiptDate,
                    fp.Payment,
                    PayAgainst=fp.PayAgainst.ToString(),             
                    ModeName = Mode[fp.Mode] + (fp.Mode == 1 ? "(" + (fp.Type == 1 ? "Depositor" : "Receiver") + " By :- " + fp.Depositor_Receiver + ")" : (fp.Mode==6?"": "(No. : " + fp.Cheque_DD_Reference_Receipt_No + ")")),
                    LetterModeName = letterMode[fp.LetterMode],
                    fp.Cheque_DD_Reference_Receipt_No,
                    fp.Depositor_Receiver,
                    fp.FavourOfId,
                    fp.DrawnOnBankId,
                    fp.DepositOnBankId,
                    LetterMode=fp.LetterMode.ToString(),
                    fp.Type,
                    PaymentDate =fp.PaymentDate.ToString("dd-MMM-yyyy"),
                    CreatedByName = _dbContext.ERPusers.FirstOrDefault(er => er.Id == fp.CreatedBy).UserName,
                    TypeName= Enum.GetName(typeof(PaymentType), fp.Type),
                    IsEdit=fp.CreationDate.Date.Equals(DateTime.Now.Date),
                    fp.RowVersion
                })
                //,
        ...
        {           
            var data = _dbContext.EventManagements.Where(x => x.SchId == SchoolId).ToList().Select(x => new
            {
                ...
                Students  = x.EnrollmentOrder.Where(o => o.IsConfirm == true).Sum(ss => ss.EnrollmentOrderDetail.Sum(eod => eod.No_Of_Student)),
                NetBalance = x.EnrollmentOrder.Where(o => o.IsConfirm == true).Sum(ss => ss.EnrollmentOrderDetail.Sum(eod => eod.No_Of_Student)) * (_dbContext.EventYears.Where(ey => ey.Event_Year == x.EventManagementYear && ey.Status == true && ey.EventId == x.EventId).Select(ey => new { EventFee = ey.EventFee - ey.RetainFee }).FirstOrDefault().EventFee),
