...
namespace SilverzoneERP.Api.api.Site
{
    public class BookController : ApiController
    {
        ...
        [HttpPost]
        public IHttpActionResult getbook_bundles(BundleList_ViewModel model)
        {
            var bundles = BundleViewModel.Parse(
                              bundleRepository.FindBy(x => x.ClassId == model.classId && x.Status == true),
                              model.subjectId
                                );

            var a = bundles.Where(x => x.bookInfo.Count() != 0);
            return Ok(new
            {
                result = a
            });
