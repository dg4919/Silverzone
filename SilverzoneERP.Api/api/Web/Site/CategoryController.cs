...
namespace SilverzoneERP.Api.api.Site
{
    public class CategoryController : ApiController
    {
        ...
        [HttpGet]
        public IHttpActionResult get_bookTitle_byClass()
        {
            // where category contains a list<a> > anonymous type 
            var category = itemTitle_MasterRepository
                           .GetAll()
                           ...
                           .Select(a => new
                           {
                               ...
                               {
                                   Id = a.Key,
                                   Name = a.FirstOrDefault().Class.className,
                                   Subject = a.GroupBy(x => x.SubjectId)
                                            .Select(b => new
                                            {
                                                Id = b.Key,
                                                Name = b.FirstOrDefault().Subject.SubjectName,
                                            })
                               }
                           });
        ...
        public IHttpActionResult get_existedBook_Title()
        {
            ...
            var books_titleId = bookRepository
                               .FindBy(x => x.Status == true)
                               .Select(x => x.Title_Mid);
