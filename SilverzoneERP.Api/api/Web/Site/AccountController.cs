...
namespace SilverzoneERP.Api.api.Site
{
    public class AccountController : ApiController
    {
        ...
        [HttpPost]
        public IHttpActionResult Register_newUser(Register_newUser_ViewModel model)
        {
            model.RoleId = 1;

            if (ModelState.IsValid && roleRepository.role_isActive(model.RoleId))
            {
                if (accountRepository.is_userExist(model.emailId, verificationMode.email, model.RoleId))
                    return Ok(new { result = "email_exist" });

                if (accountRepository.is_userExist(model.mobileNo, verificationMode.mobile, model.RoleId))
                    return Ok(new { result = "mobile_exist" });

                model.password = accountRepository.GetPasswordHash(model.password);
                var _user = new Entities.Models.User();
                Register_newUser_ViewModel.parse(model, _user);
                accountRepository.Create(_user);
        ...
        [HttpGet]
        public IHttpActionResult send_sms_onPhone_TeacherApp(string mobileNo)
        {
            string _result = string.Empty;

            // OTP is random six digit Number only.. not string !
            int _code = accountRepository.get_smsCode();
            long roleId = roleRepository.FindBy(x => x.RoleName.Trim().ToLower().Equals("teacher")).FirstOrDefault().Id;

            // if user is not registered
            if (accountRepository.FindBy(x => x.MobileNumber == mobileNo && x.RoleId == roleId).Count() == 0)
            {
                int record_count = genericOTPRepository.GetAll().Where(x => x.mobileNo == mobileNo).Count();
        ...
        public IHttpActionResult Login(LoginViewModel model)
        {
            ...
            {
                _user = accountRepository.check_User(model.userName, model.verificationMode,model.RoleId);
                ...
                if (_user != null)
                {
                    if (accountRepository.VerifyPassword(model.Password, _user.Password))
                    {
                        ...
                        if (_user.Role.RoleName.Trim().ToLower().Equals("teacher"))
                        {
                            _user.FireBaseToken = model.FireBaseToken;
                            _user.CurrentVersion = model.CurrentVersion;
                            accountRepository.Update(_user);
                        }

                        // if condison is success , get user Token &  then return from this controller
                        var userDetail = UserProfileViewModel.Parse(_user);
                        if (_user.SchCode != null)
                        {
                            var schoolDetail = _teacherDetailRepository.FindBy(x => x.Id == _user.Id).Select(x => new {
                                x.SchoolName
                            }).FirstOrDefault();

                            if (schoolDetail != null)
                                userDetail.SchoolName = schoolDetail.SchoolName;
                        }

                        // if condison is success , get user Token &  then return from this controller
                        return Ok(new { result = "ok", user = userDetail, token = getToken(model) });
                    }
                    else
                    {
        ...
        public IHttpActionResult forget_password(loginModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (_user != null)
                {
                    ...
                    {
                        verificationMode = verificationMode.email,      // send OTP to email always
                        userId = _user.Id,
                        userName = _user.EmailID,                       // send to emailId
                        OTP_code = accountRepository.get_smsCode(),
                        verficationType = verficationType.forget,
                        html_template = model.html_template,
                        emailSubject = "Password Recovery - Silverzone"
                    });
        ...
        {  // shared fx 
            var entity = forgetPasswordRepository.getRecords(model.userId, verificationMode.email);
