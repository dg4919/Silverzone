...
namespace SilverzoneERP.Api.api.Admin         // make sure end with versioning no.
{
    public class BookController : ApiController
    {
        ...
        public IHttpActionResult get_bookList(book_serchViewModel model)
        {
            ...
            var data = books.ToList().Select(x => new
            {      // selected few property whuich we required !
                bookId = x.Id,
                bookTitle = x.Title,
                bookImage = image_urlResolver.getBookImage(x.BookImage),
                bookPrice = x.Price,
        ...
        public IHttpActionResult get_bundleList()
        {
            ...
            var bundles = bundleRepository.GetAll()
                .Select(x => new
                {
                    ...
                    x.bundle_totalPrice,
                    x.Class.className,
                    books_totalPrice = x.bundle_details.Sum(y => y.book.Price),
                    x.Status,
                    //x.CreatedBy,
                    //x.UpdatedBy,
                    //x.UpdationDate
        ...
        public IHttpActionResult get_bundlebyId(int bundleId)
        {
            ...
            var bundle = new
            {
                ...
                bookInfo = books.ToList().Select(model => new
                {
                    bookId = model.Id,
