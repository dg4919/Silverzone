...
namespace SilverzoneERP.Api.api.Admin
{
    public class DispatchController : ApiController
    {
        ...
        IPaymentInfoRepository paymentInfoRepository { get; set; }

        [HttpGet]
        public IHttpActionResult getordersInfo(DateTime dateFrom, DateTime dateTo, int orderType) // 0 > Not printed || 1 > Printed
        {
            if (orderType == 0)     // challan has always a pckt id so not come in this portion
            {
                var _orderResult = from Order in orderRepository.GetAll()
                                   join paymentInfo in paymentInfoRepository.GetAll()
                                   on Order.Id equals paymentInfo.RefrenceId
                                   ...
                                   where myDispatch.DispatchInfo.Packet_Id == null
                                      && (DbFunctions.TruncateTime(Order.OrderDate) >= dateFrom && DbFunctions.TruncateTime(Order.OrderDate) <= dateTo)   // DbFunctions.TruncateTime() is used to take date part only without time
                                      && paymentInfo.Payment_Status == "Success"
                                   #region
                                   //select new
                                   //{
                                   //    myDispatch,
                                   ...
                                   select new
                                   {
                                       ...
                                       order_source = myDispatch == null ? orderSourceType.Online.ToString() : myDispatch.OrderSource.ToString(),
                ...
                var _otherDispatchResult = from otherDispatch in dispatch_otherItem_addressRepository.GetAll()
                                           ...
                                           from myDispatch in Dispatch_join.DefaultIfEmpty()           // to create left join with dispatch Table
                                           where myDispatch.DispatchInfo.Packet_Id == null
                                           && otherDispatch.Dispatch_otherItem_Master.UpdationDate >= dateFrom && otherDispatch.Dispatch_otherItem_Master.UpdationDate <= dateTo
                                           select new
                                           {
                                               //otherDispatch,
                                               //myDispatch
                                               ...
                                               order_source = myDispatch == null ? orderSourceType.Other.ToString() : myDispatch.OrderSource.ToString(),
                                               packetNumber = myDispatch == null ? string.Empty : myDispatch.DispatchInfo.Packet_Id,
                                               orderType = orderSourceType.Other
                                           };

              //  var res = _orderResult.Union(_otherDispatchResult).OrderByDescending(x => x.Id);
            ...
            else   // challan info always show here coz it will have printed pckt id..
            {
                var _result = (from dispatch in dispatch_MasterRepository.GetAll()
                               ...
                               where dispatch.DispatchInfo.CreationDate >= dateFrom && dispatch.DispatchInfo.CreationDate <= dateTo
                               select new
                               {
                               ...
                               .Select(x => new
                               {
                                   ...
                                   order_source = x.dispatch.OrderSource.ToString(),
                                   packetNumber = x.dispatch.DispatchInfo.Packet_Id,
                                   orderType = x.dispatch.OrderSource
                               })
        ...
        public IHttpActionResult create_orderLabel(orderViewModel model)
        {
            using (var transaction = dispatch_MasterRepository.BeginTransaction())
            {
                try
                {
                    ...
                    var _result = new
                    {
                        packetInfo = new { packetInfo.Invoice_No, packetInfo.Id, packetInfo.Packet_Id, packetInfo.CreationDate, imgBase = orderViewModel.getbase64(barCodeImg) },     // new objects
                        orderInfo = orderInfo,
                        status = _status
                    };
        ...
        [HttpGet]
        public IHttpActionResult get_printedOrder(DateTime dateFrom, DateTime dateTo, int wheightType) // 0 > Not printed || 1 > Printed
        {
            ...
            var _result = (from dispatch in dispatchRepository.GetAll()
                           where (
                              (wheightType == 0 && !packet_bundleInfos.Any(a => a.dispatch_mId == dispatch.Id))
                           ...
                           )   // multiple switch cases inside a single ()
                              && dispatch.Packet_Consignment == null
                              && (DbFunctions.TruncateTime(dispatch.UpdationDate) >= dateFrom && DbFunctions.TruncateTime(dispatch.UpdationDate) <= dateTo)
                           select new
                           {
                               dispatch
                           }).ToList()
        ...
        [HttpGet]
        public IHttpActionResult get_wheightedOrder(DateTime dateFrom, DateTime dateTo, int consignmentType)
        {
            var list = dispatchRepository.GetAll();
            var packet_bundleInfos = packet_BundleInfoRepository.GetAll();
        ...
        [HttpGet]
        public IHttpActionResult get_orderTrackList(DateTime dateFrom, DateTime dateTo)
        {
            var list = dispatchRepository
                .FindBy(x => x.Packet_Consignment != null
                         && (DbFunctions.TruncateTime(x.UpdationDate) >= dateFrom && DbFunctions.TruncateTime(x.UpdationDate) <= dateTo))
                .ToList()
                .Select(x => new
                {
                    x.Id,
                    ...
                    order_Source = x.Dispatch_Master.OrderSource.ToString(),
                    ...
                    trackLink = orderViewModel.get_trackUrl(x),
        ...
        public IHttpActionResult ResendOrder(long dispatchId)
        {
            using (var transaction = dispatch_MasterRepository.BeginTransaction())
            {
                try
                {
                    ...
                    {
                        Id = master.Id,
                        Packet_Id = dispatchRepository.get_Packet_Number(master.Id, packetMaster.OrderSource),
                        Invoice_No = dispatchRepository.get_Invoice_Number(master.Id),
                        Order_StatusType = orderStatusType.Pending,
        ...
        public IHttpActionResult search_dispatch(search_dispatchViewModel model)
        {
            ...
            if (model.srcId != null)
                _result = _result.Where(a => a.dispatch.OrderSource == model.srcId);
