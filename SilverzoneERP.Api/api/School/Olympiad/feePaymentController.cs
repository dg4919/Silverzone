        ...
        public IHttpActionResult Fee(FeePayment model)
        {
            ...
            if(ModelState.IsValid)
            {
                try
                {
                    ...
                    if(model.Id!=0)
                    {
                        var _feePayment = _feePaymentRepository.FindById(model.Id);
                        if (_feePayment != null && BitConverter.ToInt64(model.RowVersion, 0) == BitConverter.ToInt64(_feePayment.RowVersion, 0))
                        {
                            _feePayment.Payment = model.Payment;
                            _feePayment.PayAgainst = model.PayAgainst;
                            _feePayment.InFavourOf = model.InFavourOf;
                            _feePayment.DepositOnBankId = model.DepositOnBankId;
                            _feePayment.DrawnOnBankId = model.DrawnOnBankId;
                            _feePaymentRepository.Update(_feePayment);
                        }
                    }                    
                    else
                    {
