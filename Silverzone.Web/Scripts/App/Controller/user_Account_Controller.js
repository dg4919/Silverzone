    ...
    var user_signUpControllerfn = function ($sc, $rsc, acount_svc, $modalInstance) {
        ...
        $sc.sendMessage = function (form) {     // pass form name to validate field associate with it, thats it 
            ...
            if (form.validate()) {
                ...
                acount_svc.sendSms_onEmail({ emailId: $sc.userInfo.emailId }).then(function (d) {
                    $sc.is_loading = false;
        ...
        $sc.register_User = function (form) {
            ...
            if (form.validate()) {
                acount_svc.Register_newUser($sc.userInfo).then(function (d) {
                    if (d.result === 'ok') {
                        $modalInstance.dismiss();
                    ...
                    else if (d.result === 'email_exist') {
                        $rsc.notify_fx('User Email Id already exist, Try another :(', 'warning');
                    }
                    else if (d.result === 'mobile_exist') {
                        $rsc.notify_fx('User Mobile Number already exist, Try another :(', 'warning');
                    }
                }, function () {
                    console.log('in error');
                });
