    ...
    var fn = function ($modal) {
        ...
        fac.getCustomer_InfoModal = function (totalAmt) {
            ...
            var delete_template = ' <div class="modal-header">                                         '
                                ...
                                + ' <div class="col-sm-12">                                            '
                                + '     <input type="text" name="name" class="form-control" ng-model="usrInfo.name">'
                                + ' </div> </div>                                                      '
                                + ' <div class="form-group col-md-3">                                  '
                                + ' <label class="control-label col-sm-12">Address</label>             '
                                + ' <div class="col-sm-12">                                            '
                                + '   <textarea name="address" class="form-control" ng-model="usrInfo.address"> </textarea>'
                                + ' </div> </div>                                                      '
                                + ' <div class="form-group col-md-3">                                  '
                                + ' <label class="control-label col-sm-12">Mobile</label>              '
                                + ' <div class="col-sm-12">                                            '
                                + '     <input type="text" name="mobile" class="form-control" ng-model="usrInfo.mobile">'
                                + ' </div> </div>                                                      '
                                + ' <div class="form-group col-md-3">                                  '
                                + ' <label class="control-label col-sm-12">Email Id</label>            '
                                + ' <div class="col-sm-12">                                            '
                                + '     <input type="text" name="email" class="form-control" ng-model="usrInfo.emailId">'
                                + ' </div> </div> </div>                                               '
                                ...
                                + ' <input type="text" class="form-control" ng-model="paidAmt" style="width: 146px;position: absolute;margin-left: 10px;margin-top: 5px;" /> </div>       '
        ...
        fac.get_manageSch_byCodeModal = function () {

            var delete_template = ' <div class="modal-header">                            '
                                + '  <h4 class="box-title">Enter School Codes</h4>        '
                                + ' </div>                                                '
                                + ' <div class="modal-body">                              '
                                + '  <textarea class="form-control" rows="3" ng-model="schCodes"></textarea> '
                                + ' </div>                                                '
                                + ' <div class="modal-footer ng-scope">                   '
                                + ' <span class="col-sm-3" style="padding: 0px;">         '
                                + ' <button class="btn btn-danger pull-left" ng-click="ok(false)">Delete</button></span> '
                                + ' <button class="btn btn-info pull-right" ng-click="ok(true)">Add</button> </div>     '
                                + ' </div> </div>                                                                   '

            var modalInstance = $modal.open({
                template: delete_template,
                controller: ['$scope', '$rootScope', '$uibModalInstance', function ($sc, $rsc, $modalInstance) {
                    $sc.schCodes = null;

                    var get_schCodes = function () {
                        return _.uniq(_.map(_.filter($sc.schCodes.split(','), function (item) {
                            if (!isNaN(item))      // to check value in only number or not (_.filter fn)     
                                return item;
                        }), function (item) {      // to remove white Spaces (_.map fx execute here)         
                            return item.replace(/\s/g, '');
                        }), function (item) {      // find uniq values from array
                            return item;
                        });
                    }
                    ...
                    $sc.ok = function (sts) {
                        ...
                        if (!$sc.schCodes)
                            return $rsc.notify_fx('Please enter atleat one school code !', 'info');

                        var _schIds = get_schCodes();

                        if (!_schIds.length) {
                            $rsc.notify_fx('Please enter atleast one valid numeric school code !', 'warning');
                            return;
                        }

                        $modalInstance.close({
                            schIds: _schIds,
                            sts: sts          // True for add schools / false to remove
                        });
                    }
                }],
                size: 'md',
        ...
        fac.get_bulkSchoolModal = function () {

            var modalInstance = $modal.open({
                templateUrl: '/Templates/Inventory/modal/bulkSchoolModal.html',
                controller: ['$scope', '$rootScope', '$uibModalInstance', 'RegCordService', function ($sc, $rsc, $modalInstance, svc) {
                    $sc.model = {
                        checkSts: false
                    };
                    $sc.schList = null;
                    ...
                    svc.get_cityJson()
                    .then(function (d) {
                        $sc.cityModel = d.result;
                    });

                    $sc.search = function () {
                        svc.srchSchool($sc.model)
                        .then(function (d) {
                            $sc.model.checkSts = false;
                            $sc.schList = d.result;
                        });
                    }

                    $sc.toggleCheck = function (sts) {
                        angular.forEach($sc.schList, function (entity, key) {
                            entity.isChecked = sts;
                        });
                    }

                    var get_schoolIds = function () {
                        var _schIds = [];

                        angular.forEach($sc.schList, function (entity) {
                            if (entity.isChecked)
                                _schIds.push(entity.Id);
                        });
                    ...
                    $sc.ok = function (sts) {
                        var _schIds = get_schoolIds();

                        if (!_schIds.length) {
                            $rsc.notify_fx('Please select atleast one school !', 'warning');
                            return;
                        }
