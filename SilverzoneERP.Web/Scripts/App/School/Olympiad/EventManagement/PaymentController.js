    ...
    var Payment_Fun = function ($scope, $rootScope, modalInstance, $filter, Service, SchMngt) {
        ...
        $scope.FavourOfChange = function (FavourOfId) {
            ...
            if (DrownOnBank_Filter.length != 0)
                $scope.DepositOnBank = DrownOnBank_Filter[0].DepositOnBanks;
        }
        ...
        $scope.Edit_Payment = function (data) {
            debugger;
            $scope.Payment = angular.copy(data);
            $scope.Payment.Mode = '' + $scope.Payment.Mode;
            $scope.FavourOfChange(data.FavourOfId);
        }
