...
(function () {
    var configFn = function ($urlRouterProvider, $locationProvider, $validatorProvider, localStorageServiceProvider, $httpProvider) {
        ...
        localStorageServiceProvider
            .setStorageCookie(0);       // set cookie for 1 days in OWIN startup token file
    ...
    var runFn = function ($rsc, $state, localStorageService, $injector, notifySvc, $window) {
        ...
        $rsc.$on('$stateChangeStart', function (event, toState, toParams, fromState, fromParams) {
            ...
            if (($rsc.UserInfo === '' || $rsc.UserInfo===null) && toState.name !== 'ERP_Login') {
            ...
            else if ($rsc.UserInfo !== '' && $rsc.UserInfo === null
                    && toState.name === 'ERP_Login') {
                event.preventDefault();         // v.Imp for route state change
                $state.go('ERP_Home');
            }
            else if ($rsc.UserInfo !== '' && $rsc.UserInfo === null &&   // only few roles can access this Portal
    ...
    var moduleDependencies =
       [
           ...
           'SilverZone_Teacher_App',
           'SilverZone_Report_App'
       ];
    ...
    .run(['$rootScope', '$state', 'localStorageService', '$injector', 'notificationService', '$window', runFn])
